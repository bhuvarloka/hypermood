...
export async function createGallery(
  ...
  if (imageIds.length > 0) {
    const imageRows: TablesInsert<"gallery_images">[] = imageIds.map(
      (imageId, i) => ({
    ...
    if (imagesError) {
      ...
      throw new Error(
        `Failed to add images to gallery: ${imagesError.message}`,
      );
    }
  }
...
export async function addImagesToGallery(
  ...
  const { error } = await supabase.from("gallery_images").insert(rows as never);
...
export async function reorderGalleryImages(
  ...
  const { data: existing } = (await supabase
    .from("gallery_images")
    .select("id, image_id")
    .eq("gallery_id", galleryId)) as {
    data: { id: string; image_id: string }[] | null;
  };
...
const PUBLIC_IMAGE_FIELDS =
  "id, storage_key, original_filename, width, height, mime_type, captured_at" as const;
...
export async function listGalleriesWithImageData(
  rollId?: string,
