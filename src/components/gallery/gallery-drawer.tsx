...
export function GalleryDrawer({ onClose, rollId, initialGalleryId }: Props) {
  ...
  const handleImagesChanged = useCallback((galleryId: string, images: ImageRecord[]) => {
    setGalleries((prev) =>
      prev.map((g) => g.id === galleryId ? { ...g, image_count: images.length } : g),
    )
    setView((v) =>
      v.kind === 'detail' && v.gallery.id === galleryId ? { ...v, images } : v,
    )
  }, [])
...
function GalleryDetail({
  ...
  const handleNameBlur = useCallback(() => {
    ...
    if (!trimmed || trimmed === gallery.name) { setEditingName(false); return }
    startSave(async () => {
  ...
  const togglePublic = useCallback(() => {
    startSave(async () => {
      const updated = await updateGallery(gallery.id, { is_public: !gallery.is_public })
      onUpdated(updated)
    })
          ...
          {(['masonry', 'timeline'] as const).map((l) => (
            <button
              ...
              className={`px-3 py-1 animate-swiss ${
                gallery.layout === l ? 'bg-primary-900 text-white' : 'hover:bg-primary-50'
              }`}
            >
              {l}
            </button>
